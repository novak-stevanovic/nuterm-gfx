...
/* Detaches the stage from its loop and scene, releases drawing storage, and clears the stage. */
/* Passing `NULL` has no effect. */
void ntg_stage_deinit(ntg_stage* stage);
/* Void-pointer adapter for `ntg_stage_deinit`, intended for cleanup callbacks. */
...
/* Replaces the stage scene and keeps both sides of the relationship synchronized. A scene */
/* already owned by another stage is first detached from that stage; `NULL` clears the scene. */
...
/* RETURN VALUE: `true` when the scene handles the event; otherwise `false`. */
...
/* Assigns the owning loop pointer when it changes and marks the stage dirty when attaching to a */
/* non-`NULL` loop. It does not update the opposite loop link or invoke hooks. */
void _ntg_stage_set_loop(ntg_stage* stage, ntg_loop* loop);

/* Updates stage dimensions, propagates the size to the scene, invokes the size hook, and marks */
/* the stage dirty. Drawing storage is resized later by `_ntg_stage_compose`. */
void _ntg_stage_set_size(ntg_stage* stage, struct ntg_xy size);
/* Lays out the current scene when needed and composites all scene layers into the stage drawing. */

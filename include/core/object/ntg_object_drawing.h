...
/* Resizes a drawing, preserves overlapping virtual cells, and fills newly added cells with the */
/* default full-space virtual cell. `size_cap` caps only extra reserved capacity. */

/* ERROR CODES: */
/* - `NTG_ERR_INVALID_ARG`: `drawing` is `NULL`. */
/* - `NTG_ERR_ALLOC_FAIL`: drawing storage cannot be allocated. */
/* - `NTG_ERR_UNEXPECTED`: the underlying virtual-cell resize fails for another reason, including */
/*   arithmetic overflow. */
...
/* Copies every virtual cell from `src_drawing` directly into `dest_drawing` at `dest_start_pos`. */
/* Source and destination must be different drawings. */

/* ERROR CODES: */
/* - `NTG_ERR_INVALID_ARG`: either drawing is `NULL`, or both pointers designate the same */
/*   drawing. */
/* - `NTG_ERR_OUT_OF_BOUNDS`: the nonempty source start or end is greater than or equal to the */
/*   destination size. The current check rejects a source ending exactly at the destination */
/*   boundary. */
...
/* Composites every source virtual cell over a rendered stage cell at `dest_start_pos` using */
/* `ntg_vcell_overwrite`. */
...
/* RETURN VALUE: The stored cell, or the default full-space virtual cell when the drawing is */
...
/* Writes a virtual cell in a drawing. A `NULL` drawing or an out-of-bounds position is ignored. */
static inline void

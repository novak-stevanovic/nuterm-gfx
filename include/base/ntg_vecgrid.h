...
/* Releases the grid storage and resets its size and capacity. Passing `NULL` has no effect. */
void ntg_vecgrid_deinit(ntg_vecgrid* vecgrid);

/* Resizes an untyped grid of `data_size`-byte elements while preserving overlapping storage. */
/* `modifier` controls growth, and `size_cap` caps only extra reserved capacity; a requested size */
/* may exceed it. */

/* ERROR CODES: */
/* - `NTG_ERR_INVALID_ARG`: `vecgrid` is `NULL`, `data_size` is zero, or `modifier <= 1.05`. */

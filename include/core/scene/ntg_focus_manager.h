...
/* Clears focus, releases the scope stack, and resets the manager. Passing `NULL` has no effect. */
void ntg_focus_manager_deinit(ntg_focus_manager* fm);
/* Void-pointer adapter for `ntg_focus_manager_deinit`, intended for cleanup callbacks. */
...
/* Requests focus for `object`, or clears focus when `object` is `NULL`. A non-`NULL` object must */
/* lie within the active rooted scope. Focus and unfocus hooks are notified, but their return */
/* values are ignored. */

/* RETURN VALUE: `true` when the scope permits the requested state; otherwise `false`. */
bool ntg_focus_manager_request_focus(ntg_focus_manager* fm, ntg_object* object);

/* Pushes a focus scope unless the current scope blocks pushes. A rooted scope must belong to one */
/* of the scene layers; focus is cleared after a successful push. */

/* ERROR CODES: */
/* - `NTG_ERR_INVALID_ARG`: `fm` or `scope` is `NULL`. */
/* - `NTG_ERR_SCENE_EMPTY`: a rooted scope is requested while the scene has no layers. */
...
/* Pops the active scope, restores the previous scope and its last focused object when valid, and */
/* always keeps the default scope. A `NULL` manager is ignored. */
void ntg_focus_manager_pop_scope(ntg_focus_manager* fm);

/* Returns the scope at the top of an initialized focus-manager stack. */

/* RETURN VALUE: The active scope, or `NULL` when `fm` is `NULL` or the stack is empty. */
const struct ntg_focus_scope*
ntg_focus_manager_get_active_scope(const ntg_focus_manager* fm);

/* Marks each rooted scope invalid when `removed` is equal to or lies below that scope root, then */
/* synchronizes the stack and restores or clears focus as required. A `NULL` manager is ignored. */
...
/* RETURN VALUE: `true` when the active scope handles the event; otherwise `false`. */
bool ntg_focus_manager_feed_key(ntg_focus_manager* fm, struct nt_key_event key);
/* Hit-tests the scene, applies active-scope modal and outside-click rules, then passes the mouse */
/* event to the active scope handler or eligible outside object. */

/* RETURN VALUE: `true` when the event is handled; otherwise `false`. */

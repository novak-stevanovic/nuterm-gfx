...
// 
//     if(!loop || !init_stage || (framerate == 0) || (arena_size == 0))
...
//     }
// 
//     ntg_loop_set_stage(loop, NULL, NULL);
// 
//     init_default(loop);
// }
// 
...
// 
//     loop->_arena = sarena_create(loop->_arena_size);
//     if(!loop->_arena)
...
// 
// NTG_API bool
// ntg_loop_break(ntg_loop* loop, bool abort)
// {
//     if(!loop) return true;
// 

...
/* Detaches the scene from its stage, releases focus-manager storage, and resets the scene */
/* structure. The current implementation does not explicitly unregister or detach an existing */
/* root before reset. */
void ntg_scene_deinit(ntg_scene* scene);
/* Void-pointer adapter for `ntg_scene_deinit`, intended for cleanup callbacks. */
void ntg_scene_deinit_(void* _scene);

/* Marks the scene and its stage for a new layout/composition pass. A `NULL` scene is ignored. */
...
/* Finds the highest-z scene layer under `pos`, then recursively finds the deepest object in that */
/* layer. When non-`NULL`, `out_object_pos` receives a position local to the returned object. */

/* RETURN VALUE: The hit object, or `NULL` when no object is hit or an error occurs. */
...
/* Collects the scene root and every anchored layer root, sorted by ascending `z_index`. Passing */
/* `NULL` for `out_layers` performs a count query; a limited buffer receives at most `cap` */
/* entries. */

/* RETURN VALUE: The total number of layers regardless of `cap`, or `0` for a `NULL` scene or a */
/*               scene without a root. */
size_t ntg_scene_collect_layers_by_z(
        ntg_scene* scene,
        ntg_object** out_layers,
        size_t cap);
/* Replaces the scene root and updates registration hooks. A true root is removed from its */
/* previous scene, while an anchored layer root is unanchored before becoming the new root. */
...
/* RETURN VALUE: `true` when focus handling consumes the event; otherwise `false`. */
...
/* RETURN VALUE: `true` when the hook handles the event; otherwise `false`. */
...
/* Updates scene dimensions, marks the scene dirty, and invokes the size-change hook. The root */
/* consumes the new size during a later layout pass. */
void _ntg_scene_set_size(ntg_scene* scene, struct ntg_xy size);
/* Runs all required measurement, constraint, fixup, arrangement, and draw phases for the scene */
/* root and anchored layers. */
...
/* Assigns the owning stage pointer when it changes. It does not update the opposite stage link */
/* or invoke hooks. */
void _ntg_scene_set_stage(ntg_scene* scene, ntg_stage* stage);

/* Marks every object in a tree dirty and marks the scene dirty, recursively including ordinary */
/* children and anchored subtrees. It does not invoke registration hooks. */
void _ntg_scene_add_object_tree(ntg_scene* scene, ntg_object* root);
/* Marks the scene dirty and invalidates focus for every object in a tree, recursively including */
/* ordinary children and anchored subtrees. It does not invoke unregistration hooks. */
void _ntg_scene_rm_object_tree(ntg_scene* scene, ntg_object* root);

/* Registers one root or layer-root object and invokes registration/layer hooks as applicable. */
void _ntg_scene_register(ntg_scene* scene, ntg_object* root);
/* Unregisters one root or layer-root object and invokes unregistration/layer hooks as */
/* applicable. */
void _ntg_scene_unregister(ntg_scene* scene, ntg_object* root);
/* Registers every object in a tree and invokes applicable object, scene-change, and layer hooks, */
/* recursively including ordinary children and anchored subtrees. */
void _ntg_scene_register_tree(ntg_scene* scene, ntg_object* root);
/* Unregisters every object in a tree and invokes applicable object, scene-change, and layer */
/* hooks, recursively including ordinary children and anchored subtrees. */

...
The library is built from the ground up with strong __modularity__: Scene, Stage, Focuser, Renderer, Object, etc. are all implemented as abstract types, giving the user the ability to provide their own implementation and/or mix up different implementations. The codebase was intentionally written in __OOP style__ as an exercise in applying object-oriented principles in C, which makes the process of adding new widgets intuitive and straightforward. 

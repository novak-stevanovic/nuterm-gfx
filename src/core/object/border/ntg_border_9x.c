        ...
        ntg_object_tmp_draw* out_draw)
{
    ...
    {
        for(j = 0; j < size.x; j++)
            ntg_object_tmp_draw_set(out_draw, padding, ntg_xy(j, i));
    }
    ...
    if(size.y >= 1)
    {
        ...
        {
            for(i = 0; i < size.y; i++)
                ntg_object_tmp_draw_set(out_draw, left, ntg_xy(0, i));
        }
        ...
        if(border_size.e > 0)
        {
            for(i = 0; i < size.y; i++)
            {
                ntg_object_tmp_draw_set(out_draw, right, ntg_xy(x_end, i));
            }
        }
    }
    ...
    if(size.x >= 3)
    {
        if(border_size.n > 0)
        {
            ntg_object_tmp_draw_set(out_draw, top_left, ntg_xy(0, 0));
            for(j = 1; j < x_end; j++)
                ntg_object_tmp_draw_set(out_draw, top, ntg_xy(j, 0));
            ntg_object_tmp_draw_set(out_draw, top_right, ntg_xy(x_end, 0));
        }

        if(border_size.s > 0)
        {
            ntg_object_tmp_draw_set(out_draw, bottom_left, ntg_xy(0, y_end));
            for(j = 1; j < x_end; j++)
            {
                ntg_object_tmp_draw_set(out_draw, bottom, ntg_xy(j, y_end));
            }
            ntg_object_tmp_draw_set(out_draw, bottom_right, ntg_xy(x_end, y_end));
        }
    }
    else if(size.x >= 1)
    {

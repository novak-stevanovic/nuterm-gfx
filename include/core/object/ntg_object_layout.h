...
/* Compares minimum, natural, maximum, and growth values of two one-dimensional object */
/* measurements. */

/* RETURN VALUE: `true` when every field is equal; otherwise `false`. */
...
/* Looks up the constrained size associated with `object` in a temporary size map. */

/* RETURN VALUE: The stored size, or `NTG_SIZE_MAX` when the map/object is invalid or the object */
/*               is absent. */
size_t ntg_object_size_map_get(
        const ntg_object_size_map* map,
        const ntg_object* object);

/* Stores a constrained size for an object already represented in the temporary size map. Invalid */
/* or absent entries are ignored. */
...
/* Looks up the arranged position associated with `object` in a temporary position map. */

/* RETURN VALUE: The stored position, or `NTG_XY_MAX` when the map/object is invalid or the */
/*               object is absent. */
struct ntg_xy ntg_object_pos_map_get(
        const ntg_object_pos_map* map,
        const ntg_object* object);

/* Stores an arranged position for an object already represented in the temporary position map. */
/* Invalid or absent entries are ignored. */
...
/* RETURN VALUE: The stored cell, or the default full-space virtual cell when the drawing or */
/*               position is invalid. */
...
/* Writes a virtual cell into a temporary arena-backed drawing. Invalid drawings or positions are */
/* ignored. */
...
/* ORs the selected dirty flags into this object and marks its owning scene dirty. It does not */
/* copy the flags to ancestor objects. */
...
/* RETURN VALUE: The requested two-dimensional size; `(0, 0)` for `NULL`. */
...
/* Gets minimum, natural, maximum, and growth values for one axis of the object's outer box. */

/* RETURN VALUE: The one-dimensional measurement; all zeros for a `NULL` object. */
struct ntg_object_measure 
ntg_object_get_measure(const ntg_object* object, ntg_orient orient);
/* Gets the current outer size along `orient`. */

/* RETURN VALUE: The selected size component, or `0` for a `NULL` object. */
...
/* Gets minimum, natural, maximum, and growth values for one axis of the object's content box. */
...
/* Gets the content-width constraint used during vertical measurement; horizontal measurement has */
/* no for-size constraint. */

/* RETURN VALUE: The content width for vertical orientation, `NTG_SIZE_MAX` for horizontal */
...
/* Gets minimum, natural, maximum, and growth values for one axis of the object's padding box. */
...
/* Measures one object horizontally, applies decorator and user constraints, stores changed */
/* measurements, and invalidates dependent phases and its parent when needed. */
void _ntg_object_hmeasure(ntg_object* object, sarena* arena);
/* Computes one object’s horizontal border and padding sizes, then assigns widths to its direct */
/* children through the constrain callback. */
void _ntg_object_hconstrain(ntg_object* object, sarena* arena);
/* Measures one object vertically using its resolved width, applies decorator and user */
/* constraints, and stores changed measurements. */
void _ntg_object_vmeasure(ntg_object* object, sarena* arena);
/* Computes one object’s vertical border and padding sizes, then assigns heights to its direct */
/* children through the constrain callback. */
void _ntg_object_vconstrain(ntg_object* object, sarena* arena);
/* Runs one object’s decorator and optional custom fixup state, clears its repeat flag, and marks */
/* horizontal constraint dirty when another pass is requested. */

/* RETURN VALUE: `true` when this object requests another layout pass; otherwise `false`. */
bool _ntg_object_fixup(ntg_object* object, sarena* arena);
/* Uses one object’s arrange callback to assign positions to its direct children, offsetting for */
/* decorators and clamping positions to the parent bounds. */
void _ntg_object_arrange(ntg_object* object, sarena* arena);
/* Resizes and rebuilds one object’s drawing, then marks it for rendering. A `NULL` object or */
/* arena is ignored. */
void _ntg_object_draw(ntg_object* object, sarena* arena);

/* Sets the root width and invalidates dependent vertical layout when it changes. */
...
/* Assigns the position of a root object. It does not mark any phase dirty; non-root or `NULL` */
/* objects are ignored. */
void _ntg_object_root_set_pos(ntg_object* object, struct ntg_xy pos);

/* Clears the selected dirty bits on one object. It does not recurse into descendants. */

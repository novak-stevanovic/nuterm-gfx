#include "ntg.h"
#include <stdlib.h>
#include "shared/ntg_shared_internal.h"

/* ========================================================================== */
/* -------------------------------------------------------------------------- */
/* STATIC */
/* -------------------------------------------------------------------------- */
/* ========================================================================== */

/* ========================================================================== */
/* FUNCTIONS */
/* ========================================================================== */

static inline size_t calc_total_spacing(size_t spacing, size_t child_count)
{
    return (child_count > 0) ? ((child_count - 1) * spacing) : 0;
}

/* ========================================================================== */
/* -------------------------------------------------------------------------- */
/* PUBLIC */
/* -------------------------------------------------------------------------- */
/* ========================================================================== */

/* ========================================================================== */
/* TYPES */
/* ========================================================================== */

/* ------------------------------------------------------ */

/* ========================================================================== */
/* FUNCTIONS */
/* ========================================================================== */

/* ------------------------------------------------------ */
/* INIT/DEINIT */
/* ------------------------------------------------------ */

int ntg_box_init(ntg_box* box, const struct ntg_box_opts* opts)
{
    if(!box) return NTG_ERR_INV_ARG;

    int status = ntg_box_init_inherit(box, &NTG_BOX_VTABLE, &NTG_TYPE_BOX, NULL);
    NTG_POST_INHERIT_CHECK(status);

    ntg_box_set_opts(box, opts);

    return status;
}

/* ------------------------------------------------------ */

int ntg_box_deinit(ntg_box* box)
{
    if(!box) return NTG_ERR_INV_ARG;

    ntg_object_zero(box);
    ntg_widget_deinit((ntg_widget*)box);

    return 0;
}

/* ------------------------------------------------------ */
/* OPTS */
/* ------------------------------------------------------ */

int ntg_box_set_opts(ntg_box* box, const struct ntg_box_opts* opts)
{
    if(!box) return NTG_ERR_INV_ARG;

    struct ntg_box_opts opts_final = (opts ? (*opts) : NTG_BOX_OPTS_ZERO);

    if((box->ro.opts.orient == opts_final.orient) &&
       (box->ro.opts.prim_align == opts_final.prim_align) &&
       (box->ro.opts.sec_align == opts_final.sec_align) &&
       (box->ro.opts.spacing == opts_final.spacing) &&
       ntg_vcell_are_eql(box->ro.opts.bg, opts_final.bg))
    {
        return 0;
    }

    box->ro.opts.orient = opts_final.orient;
    box->ro.opts.prim_align = opts_final.prim_align;
    box->ro.opts.sec_align = opts_final.sec_align;
    box->ro.opts.spacing = opts_final.spacing;
    box->ro.opts.bg = opts_final.bg;

    ntg_widget_set_base_bg(ntg_wgt(box), opts_final.bg);
    ntg_widget_mark_dirty(ntg_wgt(box), NTG_WIDGET_DIRTY_FULL);

    return 0;
}

/* ------------------------------------------------------ */
/* CHILDREN */
/* ------------------------------------------------------ */

const struct ntg_widget_vec* ntg_box_get_children(const ntg_box* box)
{
    if(!box) return NULL;

    return &(((ntg_widget*)box)->ro.children);
}

/* ------------------------------------------------------ */

int ntg_box_add_child(ntg_box* box, ntg_widget* child)
{
    if(!box || !child)
        return NTG_ERR_INV_ARG;

    int _status = ntg_widget_attach((ntg_widget*)box, child);
    if(_status != 0)
        return _status;

    ntg_widget_mark_dirty((ntg_widget*)box, NTG_WIDGET_DIRTY_FULL);

    struct ntg_event_box_chldadd_dt event_dt = { .child = child };
    ntg_object_event_raise(ntg_obj(box), NTG_EVENT_BOX_CHLDADD, &event_dt);

    return 0;
}

/* ------------------------------------------------------ */

int ntg_box_rm_child(ntg_box* box, ntg_widget* child)
{
    if(!box || !child) return NTG_ERR_INV_ARG;

    if(child->ro.parent != ntg_wgt(box))
        return 0;

    ntg_widget_detach(child);

    ntg_widget_mark_dirty((ntg_widget*)box, NTG_WIDGET_DIRTY_FULL);

    struct ntg_event_box_chldrm_dt event_dt = { .child = child };
    ntg_object_event_raise(ntg_obj(box), NTG_EVENT_BOX_CHLDRM, &event_dt);

    return 0;
}

/* ========================================================================== */
/* -------------------------------------------------------------------------- */
/* PROTECTED */
/* -------------------------------------------------------------------------- */
/* ========================================================================== */

/* ========================================================================== */
/* FUNCTIONS */
/* ========================================================================== */

int ntg_box_init_inherit(
        ntg_box* box,
        const struct ntg_box_vtable* vtable,
        const ntg_type* type,
        struct ntg_widget_layout_dt* layout_dt)
{
    if(!box || !type || !vtable)
        return NTG_ERR_INV_ARG;

    if(!ntg_type_instanceof(type, &NTG_TYPE_BOX))
        return NTG_ERR_BAD_TYPE;

    int status = ntg_widget_init_inherit(ntg_wgt(box), &vtable->base, type, layout_dt);
    NTG_POST_INHERIT_CHECK_VTABLE(status);

    ntg_object_zero(box);

    return 0;
}

/* ------------------------------------------------------ */
/* IMPLEMENT */
/* ------------------------------------------------------ */

const struct ntg_box_vtable NTG_BOX_VTABLE = {
    .base = {
        .base = {
            .deinit_fn = ntg_box_deinit_fn
        },
        .measure_fn = ntg_box_measure_fn,
        .constrain_fn = ntg_box_constrain_fn,
        .arrange_fn = ntg_box_arrange_fn,
        .rm_child_fn = ntg_box_child_rm_fn
    }
};

int ntg_box_measure_fn(
        const ntg_widget* _box,
        struct ntg_widget_layout_dt* layout_dt,
        enum ntg_orient orient,
        sarena* arena,
        uint32_t* relayout,
        struct ntg_widget_measure* out_measure)
{
    (void)layout_dt;
    (void)arena;
    (void)relayout;
    const ntg_box* box = (const ntg_box*)_box;
    const struct ntg_widget_vec* children = ntg_box_get_children(box);

    if(!out_measure)
        return NTG_ERR_INV_ARG;

    if(children->size == 0)
    {
        *out_measure = (struct ntg_widget_measure) {0};
        return 0;
    }

    size_t min_size = 0, nat_size = 0, max_size = 0;

    size_t i;
    struct ntg_widget_measure it_measure;
    const ntg_widget* it_child;
    for(i = 0; i < children->size; i++)
    {
        it_child = children->data[i];
        it_measure = ntg_widget_get_measure(it_child, orient);

        
        it_measure.nat_size = ntg_max2_size(it_measure.nat_size, 1);
        it_measure.max_size = ntg_max2_size(it_measure.max_size, 1);

        if(orient == box->ro.opts.orient)
        {
            min_size += it_measure.min_size;
            nat_size += it_measure.nat_size;
            max_size += it_measure.max_size;
        }
        else
        {
            min_size = ntg_max2_size(min_size, it_measure.min_size);
            nat_size = ntg_max2_size(nat_size, it_measure.nat_size);
            max_size = ntg_max2_size(max_size, it_measure.max_size);
        }
    }

    size_t spacing = calc_total_spacing(
            box->ro.opts.spacing, children->size);

     struct ntg_widget_measure measure = {
        .min_size = min_size,
        .nat_size = nat_size + spacing,
        .max_size = NTG_SIZE_MAX,
        .grow = 1
    };

    *out_measure = measure;
    return 0;
}

/* ------------------------------------------------------ */

int ntg_box_constrain_fn(
        const ntg_widget* _box,
        struct ntg_widget_layout_dt* layout_dt,
        enum ntg_orient orient,
        ntg_widget_size_map* out_size_map,
        sarena* arena,
        uint32_t* relayout)
{
    (void)layout_dt;
    (void)arena;
    (void)relayout;
    const ntg_box* box = (const ntg_box*)_box;
    const struct ntg_widget_vec* children = ntg_box_get_children(box);
    size_t size = ntg_widget_get_size_1d_cont(_box, orient);

    if(children->size == 0) return 0;
    if(size == 0)
    {
        ntg_widget_zero_constrain(_box, out_size_map);
        return 0;
    }

    int _status;

    struct ntg_widget_measure cont_measure = ntg_widget_get_measure_cont(_box, orient);
    size_t min_size = cont_measure.min_size;
    size_t nat_size = cont_measure.nat_size;

    size_t extra_size;
    size_t array_size = children->size * sizeof(size_t);
    size_t* caps = (size_t*)sarena_malloc(arena, array_size);
    if(!caps)
        return NTG_ERR_ALLOC_FAIL;
    size_t* _sizes = (size_t*)sarena_malloc(arena, array_size);
    if(!_sizes)
        return NTG_ERR_ALLOC_FAIL;
    double* scratch_buffer = (double*)sarena_malloc(
            arena, children->size * sizeof(double));
    if(!scratch_buffer)
        return NTG_ERR_ALLOC_FAIL;
    size_t* grows = NULL;

    const ntg_widget* it_child;
    struct ntg_widget_measure it_measure;
    size_t i;
    if(orient == box->ro.opts.orient)
    {
        if(size >= nat_size) 
        {
            grows = (size_t*)sarena_malloc(arena, array_size);
            if(!grows)
                return NTG_ERR_ALLOC_FAIL;
            extra_size = size - nat_size; 

            for(i = 0; i < children->size; i++)
            {
                it_child = children->data[i];
                it_measure = ntg_widget_get_measure(it_child, orient);

                caps[i] = it_measure.max_size;
                _sizes[i] = it_measure.nat_size;
                grows[i] = it_measure.grow;
            }
        }
        else
        {
            if(size >= min_size) 
            {
                size_t pref_spacing = calc_total_spacing(
                        box->ro.opts.spacing, children->size);
                extra_size = ntg_sub2_size(size - min_size, pref_spacing);

                for(i = 0; i < children->size; i++)
                {
                    it_child = children->data[i];
                    it_measure = ntg_widget_get_measure(it_child, orient);

                    caps[i] = it_measure.nat_size;
                    _sizes[i] = it_measure.min_size;
                }
            }
            else 
            {
                extra_size = size;

                for(i = 0; i < children->size; i++)
                {
                    it_child = children->data[i];
                    it_measure = ntg_widget_get_measure(it_child, orient);

                    caps[i] = it_measure.min_size;
                    _sizes[i] = 0;
                }
            }
        }

        _status = ntg_sap_cap_round_robin(
                caps, grows, extra_size, children->size,
                scratch_buffer, _sizes, NULL);
        if(_status != 0)
            return _status;

        for(i = 0; i < children->size; i++)
        {
            it_child = children->data[i];

            ntg_widget_size_map_set(out_size_map, it_child, _sizes[i]);
        }
    }
    else
    {
        size_t it_size;

        for(i = 0; i < children->size; i++)
        {
            it_child = children->data[i];
            it_measure = ntg_widget_get_measure(it_child, orient);

            it_size =ntg_min2_size(size,
                    (it_measure.grow > 0 ?
                     it_measure.max_size :
                     it_measure.nat_size));
            it_size = (it_size > 0) ? it_size : (size > 0 ? 1 : 0);

            ntg_widget_size_map_set(out_size_map, it_child, it_size);
        }
    }

    return 0;
}

/* ------------------------------------------------------ */

int ntg_box_arrange_fn(
        const ntg_widget* _box,
        struct ntg_widget_layout_dt* layout_dt,
        ntg_widget_pos_map* out_pos_map,
        sarena* arena,
        uint32_t* relayout)
{
    (void)layout_dt;
    (void)arena;
    (void)relayout;
    const ntg_box* box = (const ntg_box*)_box;
    const struct ntg_widget_vec* children = ntg_box_get_children(box);
    ntg_xy size = ntg_widget_get_size_cont(_box);

    if(children->size == 0) return 0;
    if(ntg_xy_is_zero_any(size))
    {
        ntg_widget_zero_arrange(_box, out_pos_map);
        return 0;
    }

    int _status;
    
    enum ntg_orient orient = box->ro.opts.orient;
    enum ntg_align prim_align = box->ro.opts.prim_align;
    enum ntg_align sec_align = box->ro.opts.sec_align;

    size_t size_ort[2] = { 
        ntg_xy_get(size, orient),
        ntg_xy_get_other(size, orient)
    };

    size_t children_size_ort[2] = {0, 0};

    size_t i;
    size_t it_size_ort[2];
    const ntg_widget* it_child;
    for(i = 0; i < children->size; i++)
    {
        it_child = children->data[i];

        it_size_ort[0] = ntg_xy_get(it_child->ro.size, orient);
        it_size_ort[1] = ntg_xy_get_other(it_child->ro.size, orient);

        children_size_ort[0] += it_size_ort[0];
        children_size_ort[1] = ntg_max2_size(children_size_ort[1], it_size_ort[1]);
    }
    
    size_t pref_spacing = calc_total_spacing(box->ro.opts.spacing, children->size);
    size_t total_spacing = ntg_min2_size(
            pref_spacing,
            ntg_sub2_size(size_ort[0], children_size_ort[0]));
    
    size_t array_size = children->size * sizeof(size_t);

    size_t* spacing_caps = (size_t*)sarena_malloc(arena, array_size);
    if(!spacing_caps)
        return NTG_ERR_ALLOC_FAIL;
    for(i = 0; i < (children->size - 1); i++)
        spacing_caps[i] = NTG_SIZE_MAX;

    size_t* _spacing_after = (size_t*)sarena_calloc(arena, array_size);
    if(!_spacing_after)
        return NTG_ERR_ALLOC_FAIL;
    size_t sap_count = children->size - 1;
    double* scratch_buffer = NULL;
    if(sap_count > 0)
    {
        scratch_buffer = (double*)sarena_malloc(
                arena, sap_count * sizeof(double));
        if(!scratch_buffer)
            return NTG_ERR_ALLOC_FAIL;
    }

    _status = ntg_sap_cap_round_robin(spacing_caps, NULL, total_spacing, sap_count,
            scratch_buffer, _spacing_after, NULL);
    if(_status != 0)
        return _status;
    
    size_t cont_size_ort[2] = { 
        children_size_ort[0] + total_spacing,
        children_size_ort[1]
    };
    
    size_t base_offset_ort[2] = {
        ntg_align_offset_size(cont_size_ort[0], size_ort[0], prim_align),
        ntg_align_offset_size(cont_size_ort[1], size_ort[1], sec_align),
    };

    size_t it_extra_offset_ort[2] = {0, 0};
    ntg_xy it_pos;
    for(i = 0; i < children->size; i++)
    {
        it_child = children->data[i];

        it_size_ort[0] = ntg_xy_get(it_child->ro.size, orient);
        it_size_ort[1] = ntg_xy_get_other(it_child->ro.size, orient);
        
        it_extra_offset_ort[1] = ntg_align_offset_size(
                cont_size_ort[1],
                it_size_ort[1],
                sec_align);
        
        it_pos = ntg_xy_add(
            ntg_xy_new_orient(base_offset_ort[0], base_offset_ort[1], orient),
            ntg_xy_new_orient(it_extra_offset_ort[0], it_extra_offset_ort[1], orient)
        );

        ntg_widget_pos_map_set(out_pos_map, it_child, it_pos);
        
        it_extra_offset_ort[0] += it_size_ort[0] + _spacing_after[i];
    }

    return 0;
}

void ntg_box_child_rm_fn(ntg_widget* _box, ntg_widget* child)
{
    (void)child;

    ntg_box* box = (ntg_box*)_box;

    ntg_widget_mark_dirty((ntg_widget*)box, NTG_WIDGET_DIRTY_FULL);
}

void ntg_box_deinit_fn(ntg_object* _box)
{
    ntg_box_deinit(ntg_box(_box));
}

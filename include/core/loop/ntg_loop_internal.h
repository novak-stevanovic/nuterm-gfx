...
/* Initializes the internal deferred-task platform and associates it with `loop`. */

/* ERROR CODES: */
/* - `NTG_ERR_INVALID_ARG`: `platform` or `loop` is `NULL`. */
void _ntg_platform_init(ntg_platform* platform, ntg_loop* loop, int* out_status);
/* Releases the platform task queue and synchronization objects. Passing `NULL` has no effect. */
void _ntg_platform_deinit(ntg_platform* platform);
/* Queues an already constructed platform task for later execution on the loop thread. The */
/* callback is not validated and must be usable when the queue is drained. */

/* ERROR CODES: */
/* - `NTG_ERR_INVALID_ARG`: `platform` is `NULL`. */
/* - `NTG_ERR_PLATFORM_NO_LOOP`: the platform is invalidated and no longer has a loop. */
...
/* Executes and removes every deferred platform task currently queued. A `NULL` or invalid */
/* platform is ignored. */
void _ntg_platform_execute_all(ntg_platform* platform);
/* Detaches the platform from its loop so future deferred requests are rejected. Passing `NULL` */
/* has no effect. */
void _ntg_platform_invalidate(ntg_platform* platform);
/* Checks whether the platform is still associated with a loop. */

/* RETURN VALUE: `true` when usable; otherwise `false`, including for `NULL`. */
...
/* Initializes the worker queue and creates `worker_threads` threads. The `loop` pointer is */
/* stored but not validated. */

/* ERROR CODES: */
/* - `NTG_ERR_INVALID_ARG`: `task_runner` or `platform` is `NULL`, or `worker_threads` exceeds */
/*   `NTG_LOOP_WORKERS_MAX`. */
...
/* Stops the worker threads, joins them, and releases queue and synchronization resources. It can */
/* block until running tasks finish. */
void _ntg_task_runner_deinit(ntg_task_runner* task_runner);

/* Checks whether at least one worker task is currently executing; queued but not yet running */
/* tasks are not counted. */

/* RETURN VALUE: `true` while a task is executing; otherwise `false`, including for `NULL` or a */
/*               stopped runner. */
bool _ntg_task_runner_is_running(ntg_task_runner* task_runner);
/* Queues an already constructed worker task and signals the worker threads. If the runner is */
/* stopped or invalidated, the request is silently ignored. */
...
/* Detaches the task runner from its loop so further task submissions are ignored or rejected. */

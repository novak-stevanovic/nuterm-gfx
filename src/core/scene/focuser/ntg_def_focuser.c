...
void ntg_def_focuser_init(ntg_def_focuser* focuser, ntg_scene* scene)
{
    assert(focuser != NULL);

    ntg_focuser_init((ntg_focuser*)focuser, scene, _ntg_def_focuser_dispatch_fn);
...
void ntg_def_focuser_focus(ntg_def_focuser* focuser, ntg_object* object)
{
    ...
    if(object != NULL)
    {
        assert(ntg_object_get_scene(object) == _focuser->_scene);
    }
}
